...
# CUDA_VISIBLE_DEVICES=0 python -u train_protmod.py --exp-dir experiment/PSCLnewprot-CIFAR-10 --dataset cifar10 --num-class 10 --dist-url 'tcp://localhost:10003' --multiprocessing-distributed --world-size 1 --rank 0 --seed 1 --arch resnet18 --moco_queue 8192 --prot_start 0 --lr 0.01 --wd 1e-3 --cosine --epochs 10 --loss_weight 0.5 --proto_m 0.99 --noise_rate 0.4 --noise_type 'symmetric'
import argparse
import builtins
import math
import os
...
import numpy as np
from model_protmod import PiCO
from resnet import *
from utils.utils_algo import *
from utils.utils_loss_protmod import partial_loss, SupConLoss
# from utils.prototypeupdate import prototype
...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    ngpus_per_node = torch.cuda.device_count()
    noise_score = torch.zeros(len(train_loader)*args.batch_size*ngpus_per_node)
    all_features = torch.zeros(len(train_loader)*args.batch_size*ngpus_per_node, args.low_dim)
    all_classes = torch.zeros(len(train_loader)*args.batch_size*ngpus_per_node)
    # sample_dict = {'features': all_features, 'noise_score': noise_score, 'class': all_classes}
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
        ...
        # print(labels.shape)
        cls_out, q_feature, features_cont, pseudo_score_cont, pseudo_nlabels_cont, score_prot\
        ...
        if start_upd_prot:
            ...
            all_features[i*batch_size:(i+1)*batch_size, :] = q_feature
        ...
        loss_cont = loss_cont_fn(features=features_cont, mask=mask, batch_size=batch_size)
        # classification loss
        loss_cls, noise_score_bth, classes_bth = loss_fn(cls_out, index)
        if start_upd_prot:
            noise_score[i*args.batch_size:(i+1)*args.batch_size] = noise_score_bth
            all_classes[i*args.batch_size:(i+1)*args.batch_size] = classes_bth
    ...
    if start_upd_prot:
        # 原型选择和更新
        for each_class in torch.unique(all_classes):
            process_class = torch.eq(all_classes, each_class)
            class_index = torch.nonzero(process_class).squeeze()
            _, indices = noise_score[class_index].topk(8, dim=0, largest=True, sorted=True)
            # 用于原型更新的特征索引
            feature_index = class_index[indices]
            model.module.prototype_update(all_features[feature_index], int(each_class))
