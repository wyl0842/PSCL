...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'cub200':
        ...
        train_loader, noisy_label, train_sampler, test_loader = load_cifar10(batch_size=args.batch_size, noise_rate=args.noise_rate, noise_type=args.noise_type,split_per=args.split_percentage, random_seed=args.seed)
        ...
        train_loader, noisy_label, train_sampler, test_loader = load_tire(batch_size=args.batch_size, noise_rate=args.noise_rate, noise_type=args.noise_type,split_per=args.split_percentage, random_seed=args.seed, num_class=args.num_class)
        # train_loader, train_givenY, train_sampler, test_loader = load_cifar100(partial_rate=args.partial_rate, batch_size=args.batch_size, hierarchical=args.hierarchical)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.prot_start - 1:
            ...
            }, is_best=is_best, filename='{}/checkpoint_{}.pth.tar'.format(args.exp_dir, args.prot_start - 1),
...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
        ...
        if start_upd_prot:
            ...
            loss = args.loss_weight * loss_cont
            loss_cls_log.update(0)
