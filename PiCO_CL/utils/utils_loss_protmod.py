...
class partial_loss(nn.Module):
    ...
    def forward(self, outputs, index):
        ...
        ce_qp = -(final_outputs).sum(dim=1)
        h_p = -(logsm_outputs * outputs).sum(dim=1)
        gamma = 0.30
        # h_p = -1 * torch.clamp(gamma - h_p, torch.min(gamma - h_p), 0)
        T = gamma - h_p
        T = torch.where(T > 0*T, 0*T, T)
        T = torch.where(T < torch.min(gamma - h_p), torch.min(gamma - h_p), T)
        h_p = -1 * T
        noise_score = ce_qp - h_p

        classes = torch.argmax(self.confidence[index, :], dim=1)
        return average_loss, noise_score, classes
    ...
    def confidence_update(self, temp_un_conf, batch_index, batchY):
        with torch.no_grad():
            ...
            prot_pred = torch.div(prot_pred, 8).floor().long()
