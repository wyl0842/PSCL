...
parser.add_argument('--alpha', type=float, help='', default=0.1)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    noisy_label = np.argmax(noisy_label, axis=1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, noisy_label, model, loss_fn, optimizer, epoch, args, logger, start_upd_prot)
...
def train(train_loader, noisy_label, model, loss_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, acc_cls, acc_proto, loss_cls_log],
    ...
    if start_upd_prot:
        with torch.no_grad():
            for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
                ...
                resnet_output = model.module.predict(X_w, args)
            
                resnet_output = resnet_output.cpu().numpy()
                
                if resnet_outputs is None:
                    resnet_outputs = resnet_output
                else:
                    resnet_outputs = np.vstack((resnet_outputs,resnet_output))
        
        y_pseudo = smp.produce_pseudo_labels(resnet_outputs,noisy_label, args.num_class)
    ...
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
        ...
        # classification loss
        if start_upd_prot:
            batch_y_pseudo = y_pseudo[i*batch_size:(i+1)*batch_size].cuda()
            loss_cls = (1-args.alpha)*loss_fn(cls_out, Y)+args.alpha*loss_fn(cls_out, batch_y_pseudo)
        else:
            loss_cls = loss_fn(cls_out, Y)
            # warm up ended
