...
def load_cifar10(batch_size, noise_type='symmetric', noise_rate=0.5, split_per=0.9, random_seed=1, num_class=10):
    # temp_train = dsets.CIFAR10(root='./data', train=True, download=True, transform=transforms.ToTensor())
    # data, labels = temp_train.data, torch.Tensor(temp_train.targets).long()
    # get original data and labels
    ...
    train_data, val_data, train_labels, val_labels, ori_train_labels, ori_val_labels = tools.dataset_split(original_images, 
                                                                             original_labels, 'cifar10', noise_type, noise_rate, split_per, random_seed, num_class)
    ...
    noisy_dataset = CIFAR10_Augmentention(train_data, train_labels, original_labels)
    ...
    test_dataset = CIFAR10_Augmentention_test(test_data, test_labels)
    # test_dataset = dsets.CIFAR10(root='./data', train=False, transform=test_transform)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size*4, shuffle=False, num_workers=4,
        sampler=torch.utils.data.distributed.DistributedSampler(test_dataset, shuffle=False))
    # print(ori_train_labels.shape)
    # set test dataloader
    return train_loader, train_labels, train_sampler, test_loader, ori_train_labels
