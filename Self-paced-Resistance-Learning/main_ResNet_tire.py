...
from model.resnet_tire_2 import ResNet18 as ResNet
...
parser.add_argument('-nb', '--num_class', default=7, type=int, metavar='N', help='Number of binary bits to train (default: 8)')
parser.add_argument('--seed', type=int, default=123, help='random seed to use. Default=123')
parser.add_argument('--data_path', default='/home/wangyl/Code/CAAM/data', type=str, help='path to dataset')
...
def generate_data_weight(model, dataloader_test, epoch, b_epoch, s0=2, gamma_d=30, use_gpu=True):
    ...
    for iteration, (data, labels, indexes) in enumerate(dataloader_test, 0):
        # inputs, labels, index = data['image'], data['labels'], data['index']
        inputs, labels, index = data, labels, indexes
...
def predict(model, dataloader, batch_size=50, use_gpu=True):
    ...
    for iteration, (data, labels, indexes) in enumerate(dataloader, 0):
...
def train(model, optimizer,dataloader, test_loader, dataloader_test, true_label, noisy_rate):
    ...
    io.savemat('tire_symmetric_ResNet18_200epoch80_test_accuracy'+'_'+ str(noisy_rate)+'.mat', mdict={'test_accuracy': epoch_test_accuracy})
...
def main():
    ...
    rate = np.squeeze([0.2,0.4,0.6])

    # for iter in range(rate.size):
    for iter in range(1):

        model = ResNet(input_channel=1, num_classes=args.num_class)
        if use_gpu:
            model = model.cuda()
        ...
        train_loader_test = torch.utils.data.DataLoader(dataset=val_dataset,
                                                batch_size=args.batch_size,
                                                num_workers=args.workers,
                                                drop_last=False,
                                                shuffle=False)
        ...
        train(model, optimizer, train_loader, test_loader, train_loader_test, None, rate[iter])
